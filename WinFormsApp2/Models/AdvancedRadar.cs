...
namespace RealRadarSim.Models
{
    public class AdvancedRadar
    {
        ...
        private void InitializeAircraftMode()
        {
            ...
            double totalElevDeg = singleBarDeg * AntennaHeight;
            double totalElevRad = totalElevDeg * Math.PI / 180.0;
            minElevation = tiltOffsetRad - (totalElevRad / 2.0);
            maxElevation = tiltOffsetRad + (totalElevRad / 2.0);
        ...
        private void AdvanceElevationBar()
        {
            ...
            if (currentElevationBar >= AntennaHeight)
                currentElevationBar = 0;
            const double singleBarDeg = 2.0;
            double barSpacingRad = singleBarDeg * Math.PI / 180.0;
